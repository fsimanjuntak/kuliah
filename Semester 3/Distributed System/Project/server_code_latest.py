...
def thread_decideLeader():
	...
	try:
		...
		if (global_info.getServerStatus() in [MessageType.SERVERBUSY, MessageType.LISTOFSERVERUPDATED, MessageType.REQUESTLISTOFSERVER, MessageType.ACKNOWLEDGEMENTFROMALIVESERVER]):
			#if the server status is busy
			if (global_info.getServerStatus() in [MessageType.REQUESTLISTOFSERVER, MessageType.ACKNOWLEDGEMENTFROMALIVESERVER, MessageType.SERVERBUSY]):
				#try several times until the status has been changed to LISTOFSERVERUPDATED
				for i in range(1, 100):
					if (global_info.getServerStatus() in [MessageType.REQUESTLISTOFSERVER, MessageType.ACKNOWLEDGEMENTFROMALIVESERVER]):
						print("Re-Request list of server, attempt: %s"%(str(i)))
						...
						print("Trying to connect to network, attempt: %s"%(str(i)))
					sleep(20)
					#if the status has been changed to LISTOFSERVERUPDATED then quit from the loop
				...
				if (sender_type == SenderType.SERVER):
					if sender_port != this_server.port:
						if ((global_info.getServerStatus() in [MessageType.SERVERUP, MessageType.RUNNING,MessageType.VOTING,MessageType.REQUESTLISTOFSERVER,MessageType.ACKNOWLEDGEMENTFROMALIVESERVER,MessageType.LISTOFSERVERUPDATED, MessageType.PAUSERUNNING])):
							...
							if (message_type == MessageType.SERVERUP and (global_info.getServerStatus() == MessageType.VOTING or global_info.getServerStatus() == MessageType.PAUSERUNNING)):
								...
								# print("I sent a message to the potential server to try again since the server is busy now")

							#if the server status is up and the message type is declare the leader
							if ((global_info.getServerStatus() == MessageType.SERVERUP or global_info.getServerStatus() == MessageType.RUNNING or global_info.getServerStatus() == MessageType.VOTING) and message_type == MessageType.DECLARETHELEADER):
								print("global_info.getServerStatus()", global_info.getServerStatus(), "message type", message_type, " content", message_content)
