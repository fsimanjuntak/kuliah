...
OverallTDL <- c(LogitOutput$TDL,NearNOutput$TDL,naiveBayesOutput$TDL,SVMOutput$TDL,TreeOutput$TDL,RFOutput$TDL,NNetOutput$TDL)
OverallGINI <- c(LogitOutput$GINI,NearNOutput$GINI,naiveBayesOutput$GINI,SVMOutput$GINI,TreeOutput$GINI,RFOutput$GINI,NNetOutput$GINI)
...
barplot(t(as.matrix(ForGraph[, c("OverallTDL1", "OverallGINI1")])), beside = TRUE, yaxt = "n", names.arg = c("Logit","N. Neighbour", "Naive Bayes","SVM","Tree","Random Forest","Neural Network"), ylim=c(0, max(c(myLeftAxisAt, myRightAxisAt))), ylab =	"Top Decile Lift", legend = c("TDL","GINI"), main="Performance of the Machine Learning Algorithms")
