...
sessionleveldata <- read_delim("datasets/sessionleveldata.csv", ";", escape_double = FALSE, trim_ws = TRUE)
...
#Load datetime
MiceComplete_sessiondata$date = sessionleveldata$date
MiceComplete_sessiondata$session = sessionleveldata$session
#Load Temperature
temperature_data <- read.csv("datasets/Temperature.csv",header=TRUE) #read csv file
...
for (i in 1:nrow(MiceComplete_sessiondata)) {
  MiceComplete_sessiondata$TG_avg_temp[i] <- temperature_data$TG[temperature_data$date == MiceComplete_sessiondata$date[i]]
  MiceComplete_sessiondata$FG_avg_wind[i] <- temperature_data$FG[temperature_data$date == MiceComplete_sessiondata$date[i]]
  MiceComplete_sessiondata$SQ_sunshine[i] <- temperature_data$SQ[temperature_data$date == MiceComplete_sessiondata$date[i]]
  MiceComplete_sessiondata$RH_precipitation[i] <- temperature_data$RH[temperature_data$date == MiceComplete_sessiondata$date[i]]
  MiceComplete_sessiondata$NG_avg_cloud[i] <- temperature_data$NG[temperature_data$date == MiceComplete_sessiondata$date[i]]
  MiceComplete_sessiondata$UG_humidity[i] <- temperature_data$UG[temperature_data$date == MiceComplete_sessiondata$date[i]]
}
MiceComplete_sessiondata$IsSale <- ifelse(MiceComplete_sessiondata$sale>0,1,0)

# Write dataframe to CSV file
write.csv(MiceComplete_sessiondata, file = "datasets/MiceCompleteSessionData.csv")
